...
def compute_u_hat_and_derivative(
    *,
    ...
    """Compute CM-Bridge reference field u_hat (normalized) and its time derivative.

    u_hat_norm(t) = alpha(t) * (x_trig / sigma) + beta(t) * (v_tau / sigma)
    where v_tau = dx/dtau = -dx/dr = -v_bridge.

    Derivative is approximated by finite difference in trig time:
      du_hat/dt ≈ (u_hat(t+dt) - u_hat(t)) / dt.
    """
    t0 = t_trig.to(device=device, dtype=torch.float32)
    t1 = (t0 + fd_dt).clamp(0.0, (math.pi / 2) - 1e-6)

    alpha0, beta0 = converter.compute_vector_field_coefficients(t0)
    alpha1, beta1 = converter.compute_vector_field_coefficients(t1)

    alpha0 = alpha0.view(-1, 1, 1, 1)
    beta0 = beta0.view(-1, 1, 1, 1)
    alpha1 = alpha1.view(-1, 1, 1, 1)
    beta1 = beta1.view(-1, 1, 1, 1)

    v_bridge0 = teacher_feature(
        teacher,
        teacher_cond_proj,
        x_trig=x_trig,
        x0_cond=x0_cond,
    ...
    u_hat0 = alpha0 * x_norm + beta0 * (v_tau0 / sigma)
    u_hat1 = alpha1 * x_norm + beta1 * (v_tau1 / sigma)
...
def parse_args():
    ...
    p.add_argument("--adv_lambda", type=float, default=0.1)
    ...
    p.add_argument("--num_log_images", type=int, default=1, help="Number of images from the batch to decode and log.")
        ...
        for step, batch in enumerate(batch_iter):
            ...
            if accelerator.sync_gradients:
                ...
                if (
                    ...
                    with torch.no_grad():
                        n = int(max(1, args.num_log_images))
                        n = min(n, x0.shape[0])

                        # Teacher reference "output" is derived from u_hat (the sCM teacher signal)
                        x1_hat_teacher_norm = sin_t * z_t + cos_t * u_hat
                        x1_hat_teacher = x1_hat_teacher_norm * float(args.sigma_data)

                        # Decode (VAE expects fp32)
                        src_img = decode_latents_to_images(vae, x0[:n].float()).detach().cpu()
                        tar_img = decode_latents_to_images(vae, x1[:n].float()).detach().cpu()
                        stu_img = decode_latents_to_images(vae, x1_hat[:n].float()).detach().cpu()
                        tea_img = decode_latents_to_images(vae, x1_hat_teacher[:n].float()).detach().cpu()

                        # Build one big image: N rows, each row is [S | T | Student | Teacher]
                        rows = []
                        for i in range(n):
                            row = torch.cat(
                                [
                            ...
                            rows.append(row)
                        comp = torch.cat(rows, dim=2)  # stack rows vertically
                        tb_writer.add_images("images/comparison", comp, global_step + 1)
