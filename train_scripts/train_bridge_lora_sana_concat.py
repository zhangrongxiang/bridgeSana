...
def decode_latents(vae, latents):
    """Decode latents back to image space in [-1, 1] (approx), keeping gradients w.r.t. latents."""
    scaling_factor = getattr(getattr(vae, "config", None), "scaling_factor", None)
    if scaling_factor is not None:
        latents = latents / scaling_factor
...
def encode_prompts(tokenizer, text_encoder, prompts, device):
    ...
    return encoder_hidden_states, text_inputs.attention_mask
...
def main():
    ...
    if args.conditioning in {"concat", "concat_text"}:
        ...
        # Initialize as identity on x_t part: output ~= x_t at start
    ...
    # Setup optimizer (trainable LoRA params + optional cond_proj)
    trainable_params = [p for p in transformer.parameters() if p.requires_grad]
    if cond_proj is not None:
        trainable_params += list(cond_proj.parameters())
    ...
    if cond_proj is not None:
        transformer, cond_proj, optimizer, train_dataloader, lr_scheduler = accelerator.prepare(
            transformer, cond_proj, optimizer, train_dataloader, lr_scheduler
        )
        # Make adapter accessible for validation sampling
        accelerator.unwrap_model(transformer)._bridge_cond_proj = accelerator.unwrap_model(cond_proj)
    else:
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(transformer):
                ...
                # Compute Bridge loss
                bridge_loss = compute_bridge_loss(
                ...
                pixel_loss = None
                if args.pixel_loss_lambda and args.pixel_loss_lambda > 0:
                ...
                if pixel_loss is not None:
                    loss = loss + args.pixel_loss_lambda * pixel_loss
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % args.checkpointing_steps == 0:
                    if accelerator.is_main_process:
                        ...
                        # Save concat conditioning adapter (if any)
                        if cond_proj is not None:
        ...
        with torch.inference_mode():
            with autocast_ctx:
                if not use_concat:
                    ...
                    # Concat-conditioned sampling loop
                    encoder_hidden_states, encoder_attention_mask = None, None
                    if args.conditioning in {"text", "concat_text"}:
                        encoder_hidden_states, encoder_attention_mask = encode_prompts(
                            tokenizer, text_encoder, [args.validation_prompt], accelerator.device
                        )
                    # Grab the prepared cond_proj from the outer scope if present
                    cond_proj_module = getattr(accelerator.unwrap_model(transformer), "_bridge_cond_proj", None)
                    if cond_proj_module is None:
                        raise RuntimeError("Concat conditioning enabled but cond_proj adapter not attached to transformer.")
                    cond_proj_module.eval()

                    latents_out = sample_concat_condition(
                        transformer=accelerator.unwrap_model(transformer),
                    ...
                    img_tensor = decode_latents(vae, latents_out).clamp(-1, 1)
